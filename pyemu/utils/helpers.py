...
def apply_list_and_array_pars(arr_par_file="mult2model_info.csv", chunk_len=50):
    ...
    arr_pars.loc[:,"lower_bound"] = arr_pars.pop("lower_bound").astype(float)
    arr_pars.loc[:,"upper_bound"] = arr_pars.pop("upper_bound").astype(float)
...
def calc_array_par_summary_stats(arr_par_file="mult2model_info.csv"):
    ...
    for model_input_file in model_input_files:
        ...
        ub = df.loc[df.model_file == model_input_file, "upper_bound"].astype(float).max()
        lb = df.loc[df.model_file == model_input_file, "lower_bound"].astype(float).min()
        if pd.isna(ub):
